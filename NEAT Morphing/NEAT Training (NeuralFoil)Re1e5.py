...
def compute_fitness(genome, config, target_aoa, noise_sigma=0.0005):
    ...
    # ================= Reynolds-dependent scaling =================

    if REYNOLDS >= 1e6:
        cm_weight = 160 * (1 - 0.25 * progress)
        ld_weight = 520 * (1 + 1.1 * progress)
        cd_penalty = 35
        CL_target = 0.6
        LD_scale = 1.0

    elif REYNOLDS >= 5e5:
        cm_weight = 170 * (1 - 0.25 * progress)
        ld_weight = 500 * (1 + 1.1 * progress)
        cd_penalty = 40
        CL_target = 0.65
        LD_scale = 0.9

    elif REYNOLDS >= 3e5:
        cm_weight = 180 * (1 - 0.25 * progress)
        ld_weight = 480 * (1 + 1.1 * progress)
        cd_penalty = 45
        CL_target = 0.7
        LD_scale = 0.8

    elif REYNOLDS >= 2e5:
        cm_weight = 200 * (1 - 0.25 * progress)
        ld_weight = 460 * (1 + 1.1 * progress)
        cd_penalty = 55
        CL_target = 0.75
        LD_scale = 0.7

    elif REYNOLDS >= 1e5:
        cm_weight = 220 * (1 - 0.25 * progress)
        ld_weight = 440 * (1 + 1.1 * progress)
        cd_penalty = 65
        CL_target = 0.85
        LD_scale = 0.6

    else:
        cm_weight = 240 * (1 - 0.25 * progress)
        ld_weight = 420 * (1 + 1.1 * progress)
        cd_penalty = 75
        CL_target = 0.95
        LD_scale = 0.5
    ...
    cm_term = -cm_weight * (cm_violation / CM_LIMIT) ** 1.5

    # ================= Lift/Drag reward =================

    cl_safe = max(cl_corr, 1e-3)
    LD = (cl_safe ** 1.4) / cd_corr

    LD_norm = LD * LD_scale

    ld_term = ld_weight * np.log(LD_norm + 1) ** 1.3

    # ================= Lift requirement =================

    lift_penalty = 0
    if cl_corr < CL_target:
        lift_penalty = -60 * (CL_target - cl_corr)

    # ================= Final fitness =================

    fitness = cm_term + ld_term + lift_penalty

    fitness -= cd_penalty * cd_corr

    smooth_penalty = np.sum(np.maximum(0, np.abs(np.diff(delta_y_smooth)) - 0.05)**2)
    fitness -= 0.02 * smooth_penalty / num_ctrl
    ...
    fitness -= 0.15 * np.mean(curvature_violation ** 2)
    ...
    fitness -= 0.08 * (np.mean(d2yu_dx2[mask] ** 2) + np.mean(d2yl_dx2[mask] ** 2))
...
def train_for_aoa(target_aoa, generations=50):
    ...
    cm_weight_history, ld_weight_history = [], []
    ...
    def eval_genomes(genomes, config):
        ...
        # Evaluate all genomes and track overall best
        for gid, genome in genomes:
            genome.fitness = compute_fitness(genome, config, target_aoa)
            if genome.fitness > overall_best_fitness:
                overall_best_fitness = genome.fitness
                overall_best_genome = genome
