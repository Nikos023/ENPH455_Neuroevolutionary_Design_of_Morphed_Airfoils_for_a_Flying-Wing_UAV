...
def compute_fitness(genome, config, target_aoa, noise_sigma=0.0005):
    ...
    if REYNOLDS >= 1e6:
        cm_weight = 120 * (1 - 0.25 * progress)
        ld_weight = 20 * (1 + 1.20 * progress)
    elif REYNOLDS >= 5e5:
        cm_weight = 140 * (1 - 0.25 * progress)
        ld_weight = 40 * (1 + 1.20 * progress)
    elif REYNOLDS >= 3e5:
        cm_weight = 160 * (1 - 0.25 * progress)
        ld_weight = 60 * (1 + 1.20 * progress)
    elif REYNOLDS >= 2e5:
        cm_weight = 180 * (1 - 0.25 * progress)
        ld_weight = 80 * (1 + 1.20 * progress)
    elif REYNOLDS >= 1e5:
        cm_weight = 200 * (1 - 0.25 * progress)
        ld_weight = 100 * (1 + 1.20 * progress)
    else:
        cm_weight = 220 * (1 - 0.25 * progress)
        ld_weight = 120 * (1 + 1.20 * progress)

    cm_violation = max(0.0, abs(cm_corr) - CM_LIMIT)
    cm_term = -cm_weight * (cm_violation / CM_LIMIT)

    LD = cl_corr / (cd_corr * 2)
    LD_safe = max(LD, 1e-3)
    ld_term = ld_weight * (LD_safe ** 1.2)
